...
From the groups seperated in Problem 21, we can find that they are symmetric. If we can connect all inputs directly to output neuron, we can specify which side are we on by $\text{OR}$ all inputs. Hence we can reduce the number of neurons in hidden layer by half, which is $\Divide{\ParTh{d-1}}{2}$ for $d$ is odd and $\ParTh{\Divide{d}{2}}$ for $d$ is even, where $\Divide{\ParTh{d-1}}{2}$ is due to we do not need the central hyperplane to seperate the inputs.
