...
def main():
    ...
    Nprime   = len(TRAIN18_DATA)
    treeList = []
    for times in range(REPEAT):
        treeList = np.append(treeList, bagAlg(T, Nprime, TRAIN18_DATA))
    ...
    while idx < (T * REPEAT):
        ...
        pd[pd >= 0] = 1.0
        pd[pd <  0] = -1.0
        EGtList.append(np.mean((pd != yTes18).astype(int)))
        idx += 1
    ...
    print "============================================"
    print "Q18: Eout =", np.mean(EGtList)
    print "--------------------------------------------"
