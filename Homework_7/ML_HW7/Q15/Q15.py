...
def main():
    ...
    TRAIN15_FILE = 'hw3_train.dat'
    TRAIN15_DATA = np.loadtxt(TRAIN15_FILE, dtype=np.float)

    TEST15_FILE = 'hw3_test.dat'
    TEST15_DATA = np.loadtxt(TEST15_FILE, dtype=np.float)
    xTes15 = TEST15_DATA[:, 0:(TEST15_DATA.shape[1] - 1)]
    yTes15 = TEST15_DATA[:, (TEST15_DATA.shape[1] - 1)]
    
    deciTree = decisionTree(TRAIN15_DATA, 0, 0, 1.0, 0.0, Tree())

    t1 = time.time()

    print "============================================"
    print "Q15: Ein =", treeCalErr(xTes15, yTes15, deciTree)
    print "--------------------------------------------"
