...
def main():
    ...
    xTra19 = TRAIN19_DATA[:, 0:(TRAIN19_DATA.shape[1] - 1)]
    yTra19 = TRAIN19_DATA[:, TRAIN19_DATA.shape[1] - 1]
    
    Nprime   = len(TRAIN19_DATA)
    treeList = []
    for times in range(REPEAT):
        treeList = np.append(treeList, bagAlg(T, Nprime, TRAIN19_DATA))
    ...
    while idx < (T * REPEAT):
        ...
        pd[pd >= 0] = 1.0
        pd[pd <  0] = -1.0
        EGtList.append(np.mean((pd != yTra19).astype(int)))
        idx += 1

    repeatList = np.array(range(T * REPEAT)) + 1
    
    plotHist(repeatList, EGtList, r"$t$", r'$E_{\mathrm{in}}(G_t)$', "Q19", 1, False)

    t1 = time.time()

    print "============================================"
    print "Q19: Ein =", np.mean(EGtList)
    print "--------------------------------------------"
