...
def main():
    ...
    Nprime   = len(TRAIN17_DATA)
    treeList = []
    for times in range(REPEAT):
        treeList = np.append(treeList, bagAlg(T, Nprime, TRAIN17_DATA))
    ...
    while idx < (T * REPEAT):
        ...
        pd[pd >= 0] = 1.0
        pd[pd <  0] = -1.0
        eIn = np.mean((pd != yTra17).astype(int))
        if eIn != 0:
            EGtList.append(np.log10(eIn))
        idx += 1

    repeatList = np.array(range(len(EGtList))) + 1
    plotHist(repeatList, EGtList, r"$t$", r'$\log_{10}(E_{\mathrm{in}}(G_t))$', "Q17-log-2", 1, False)

    t1 = time.time()

    print "============================================"
    print "Q17: log(Ein) =", np.mean(EGtList)
    print "--------------------------------------------"
